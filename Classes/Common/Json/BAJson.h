...
#ifndef BROKERAPP_BAJSON_H
#define BROKERAPP_BAJSON_H
...
public:
    ...
    {
        auto array = BAJson::array();

        for (auto item : list)
            BAJson::append(document, array, item);

        BAJson::set(document, object, key, array);
    }
