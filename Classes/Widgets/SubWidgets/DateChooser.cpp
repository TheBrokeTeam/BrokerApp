...
// Tip: we modify tm (its fields can even be negative!) and then we call this method to retrieve a valid date
    ...
    size_t DateToString(tm& d,char* buf, size_t bufferSize)  {
        return strftime(buf, bufferSize, "%d/%m/%Y", &d);
    }
    ...
    long GetTimestamp(tm date, bool resetHours) {
        if(resetHours) {
            date.tm_hour = 0;
            date.tm_min = 0;
            date.tm_sec = 0;
        }
        return timelocal(&date)*1000;
    }
