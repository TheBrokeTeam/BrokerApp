...
{
    const bool no_vsync = true;
    const bool use_msaa = true;
    const bool im_style = true;
    ...
    glfwWindowHint(GLFW_RESIZABLE, GL_TRUE); //set resizable option
    ...
    if (im_style) {
        ...
        _clearColor = ImVec4(0.15f, 0.16f, 0.21f, 1.00f);
        StyeColorsApp();
    }
    ...
    _fonts[font_cfg.Name] = io.Fonts->AddFontFromMemoryTTF(Roboto_Bold_ttf, Roboto_Bold_ttf_len, 15.0f, &font_cfg);
    io.Fonts->AddFontFromMemoryTTF(fa_solid_900_ttf, fa_solid_900_ttf_len, 14.0f, &icons_config, fa_ranges);

    ImStrncpy(font_cfg.Name, "Roboto Italic", 40);
    _fonts[font_cfg.Name] = io.Fonts->AddFontFromMemoryTTF(Roboto_Italic_ttf, Roboto_Italic_ttf_len, 15.0f, &font_cfg);
    io.Fonts->AddFontFromMemoryTTF(fa_solid_900_ttf, fa_solid_900_ttf_len, 14.0f, &icons_config, fa_ranges);
